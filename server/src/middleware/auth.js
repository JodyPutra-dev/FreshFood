...
function verifyApiKey(req, res, next) {
  try {
    ...
    if (!expectedKey || expectedKey === 'your-secret-api-key-here-change-this-in-production') {
      console.error('ADMIN_API_KEY not properly configured');
    ...
    if (!providedKey) {
      console.warn('Authentication attempt without API key');
    ...
    if (typeof providedKey !== 'string' || providedKey.length < 16) {
      console.warn('Authentication attempt with invalid API key format');
    ...
    if (providedKeyBuffer.length !== expectedKeyBuffer.length) {
      console.warn(`Authentication failed: Key length mismatch`);
      return res.status(401).json({
        error: 'Unauthorized',
